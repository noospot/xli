...
static bool CutBox( Box* box, Box* newbox );
static int GreatestVariance( Box* boxes, int n );
static bool FindCutpoint( Box* box, int color, Box* newbox1, Box* newbox2 );
...
Image* reduce( Image* image, unsigned colors, int ditherf, float gamma, int verbose )
{
	...
	switch ( image->type ) {
		...
		default: {
			lfree( ( byte* ) Histogram );
			lfree( ( byte* ) Boxes );
			lfree( ( byte* ) rgbmap );
			return ( image );   /* not something we can reduce, thanks anyway */
		}
	}
	...
	new_image->rgb.compacted_cmap = TRUE;
...
static void UpdateFrequencies( Box* box1, Box* box2 )
{
	...
	memset( b1f0, 0, ColormaxI * sizeof( unsigned long ) );
	memset( b1f1, 0, ColormaxI * sizeof( unsigned long ) );
	memset( b1f2, 0, ColormaxI * sizeof( unsigned long ) );
                           ...
                           int ditherf )
{
	...
	if ( !ditherf ) {
		 ...
		 */
		memset( ( byte* ) rgbmap, 0xff, ColormaxI * ColormaxI * ColormaxI
		        * sizeof( *rgbmap ) );
                            ...
                            int verbose )
{
	...
	if ( !ditherf ) {
		...
		if ( UNSET_GAMMA == gamma || ( gamma / inimage->gamma ) == 1.0 )
			for ( x = 0; x < ( long int )outimage->rgb.size; x++ ) {
