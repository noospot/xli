...
static byte* BitRow;			/* working row of bits */
static int RowRepeat;				/* number of repititons */
static unsigned int RowCount;				/* current row */
static unsigned int ColCount;				/* current column */

static int IMG_ReadHeader( ZFILE* zf, IMG_Header* h );			/* Read file header */
static void IMG_WriteByte( byte c, register int cols, unsigned int bpl );			/* Write output byte */
...
Image* imgLoad ( strbyte* fullname, ImageOptions* image_ops, bool verbose )
{
	...
	byte inbuf[MAX_LITERAL], ichr;	        /* bytes in */
...
static int IMG_ReadHeader ( ZFILE* zf, IMG_Header* h )
{
	...
	byte junkbuffer[MAX_SCANLINE];	/* scrap buffer */
...
static void IMG_WriteByte ( byte c, register int cols, unsigned int bpl )
{
	( void )cols;
