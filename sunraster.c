...
static void babble( strbyte* name, struct rheader* header )
{
	printf( "%s is a ", name );
...
static bool sunread( ZFILE* zf, byte* buf, unsigned int len, unsigned int enc, strbyte* name )
{
	static byte rbyte, remaining= 0;
	...
	if ( enc ) {
		...
		if ( zread( zf, buf, len ) < ( long int )len ) {
...
Image* sunRasterLoad( strbyte* fullname, ImageOptions* image_ops, bool verbose )
{
	...
	unsigned int    fillnum;   /* # of fill bytes per raster line */
	unsigned int    enc;
	byte            fillbyte;
	Image*          image;
	byte*           lineptr;
	...
	if ( mapsize ) {
		map= lmalloc( mapsize );
		if ( zread( zf, map, mapsize ) < ( long int )mapsize ) {
	...
	if ( depth >= 24 ) {
		...
		for ( y= 0; y < image->height; y++ ) {
			...
			if ( fillnum )
				if ( !sunread( zf, &fillbyte, fillnum, enc, name ) ) {
					lfree( buf );
					zclose( zf );
					return( image );
				}
