...
Image* halftone( Image* cimage, unsigned int verbose )
{
	...
	if ( cimage->title ) {
		image->title = ( strbyte* ) lmalloc( strlen( cimage->title ) + 13 );
	...
	for ( y = 0; y < cimage->height; y++ ) {
		for ( x = 0; x < cimage->width; x++ ) {
			 ...
			 * putting all four X bits in at once.  
			 * if it seems this would be hard to change to be an
			 * NxN dithering array, that's right, since we're
