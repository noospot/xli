/* compact.c:
 *
 * compact an index colormap by removing unused or duplicate RGB
...
void compact_cmap( Image* image, unsigned int verbose )
{
	...
	if ( verbose ) {
		printf( "  Compacting colormap..." );
		fflush( stdout );
	}
	used = ( byte* ) lcalloc( sizeof( byte ) * depthToColors( image->depth ) );
	...
	image->rgb.compacted_cmap = TRUE;	/* don't do it again */
}
