...
extern struct sv_globals {
	...
	strbyte**     sv_comments;	/* pointer to array of pointers to comments */
...
#define SV_BIT(glob,bit) \
	...
	union {
		struct {
			int	scan_y,
			      vert_skip;
			byte is_eof,	/* Set when EOF or EofOp encountered */
 ...
 */
extern strbyte* rle_getcom( strbyte* name, struct sv_globals* globals );
