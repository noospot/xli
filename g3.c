 ...
 ** string.  the old errors are currently ifdef'ed out -- if wanting them,
...
tryagain:
	...
	if ( col1 > 0 && !g3_error ) {
		col3 = g3_getfaxrow( fd, tmpline );
	}
	if ( !g3_error && col1 > 0 && col1 == col2 && col2 == col3 ) {
		ret = TRUE;
	} else {
		ret = FALSE;
	}
