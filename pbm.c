...
static int pbmReadStrByte( ZFILE* zf )
{
	int c;
...
static int pbmReadStrInt( ZFILE* zf )
{
	int c, value;
...
static int pbmReadRawInt( ZFILE* zf, int depth )
{
	int i, src, src2;

	src = 0;
	for ( i = 0; i < depth; i += 8 ) {
		if ( ( src2 = zgetc( zf ) ) == EOF ) {
			return EOF;
		}
		src = ( src << 8 ) | src2;
	}
...
static int isPBM( ZFILE* zf, strbyte* name, int* width, int* height, int* maxval,
                  unsigned int verbose )
{
	...
	if ( memToVal( ( byte* ) buf, 2 ) == memToVal( ( byte* ) "P1", 2 ) ) {
		if ( ( ( *width = pbmReadStrInt( zf ) ) < 0 ) ||
		      ( ( *height = pbmReadStrInt( zf ) ) < 0 ) ) {
			return ( NOTPBM );
		}
	...
	if ( memToVal( buf, 2 ) == memToVal( ( byte* ) "P2", 2 ) ) {
		if ( ( ( *width = pbmReadStrInt( zf ) ) < 0 )
		      || ( ( *height = pbmReadStrInt( zf ) ) < 0 )
		      || ( ( *maxval = pbmReadStrInt( zf ) ) < 0 ) ) {
			return ( NOTPBM );
		}

		if ( verbose ) {
			printf( "%s is a %dx%d PGM image with %d levels\n",
			        name, *width, *height, ( *maxval + 1 ) );
		}
		return ( PGMNORMAL );
	}
...
Image* pbmLoad( strbyte* fullname, ImageOptions* image_ops, bool verbose )
{
	...
	int pbm_type;
	int x, y;
	int width, height, maxval, fmaxval;
	unsigned int depth, fdepth;
	...
	switch ( pbm_type ) {
		case PBMNORMAL:
			...
			for ( y = 0; y < height; y++ ) {
				...
				for ( x = 0; x < width; x++ ) {
					do {
						if ( ( src = pbmReadStrByte( zf ) ) < 0 ) {
							fprintf( stderr, "pbmLoad: %s - Short image\n", name );
							zclose( zf );
							return ( image );
						}
						if ( IntTable[src] == NOTINT ) {
							fprintf( stderr, "pbmLoad: %s - Bad image data\n", name );
							zclose( zf );
							return ( image );
						}
		...
		case PGMRAWBITS:
			...
			//image->gamma = DEFAULT_DISPLAY_GAMMA;	/* xli default */
			image->gamma = image_ops->gamma;
			image->title = dupString( name );
			size = height * width;
