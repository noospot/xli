...
/* rotate()
 * rotates an imageby 90, 180, or 270 degrees.
 */
Image* rotate( Image* iimage, int rotate, int verbose )
{
	...
	for ( rot = 0; rot < rotate; rot += 90 ) {
		switch ( simage->type ) {
			...
			case IRGB:
				...
				yptr = ( byte** ) lmalloc( simage->width * sizeof( byte* ) );
	...
	dimage->title = ( strbyte* ) lmalloc( strlen( iimage->title ) + 40 );
		...
		case ITRUE:
			...
			for ( int row = 0; row < height; row++ ) {
				for ( int col = 0; col < width / 2; col++ ) {
					int left_offset = ( row * width + col )*3;
					int right_offset = ( row * width + ( width - 1 - col ) )*3;
					// Swap pixels on the left and right sides of the row
					byte temp = image->data[left_offset];
					image->data[left_offset] = image->data[right_offset];
					image->data[right_offset] = temp;
					temp = image->data[left_offset+1];
					image->data[left_offset+1] = image->data[right_offset+1];
					image->data[right_offset+1] = temp;
					temp = image->data[left_offset+2];
					image->data[left_offset+2] = image->data[right_offset+2];
					image->data[right_offset+2] = temp;
				}
			}
			break;
		default:
		...
		case ITRUE:
			for ( int row = 0; row < height / 2; row++ ) {
				for ( int col = 0; col < width; col++ ) {
					int top_offset = ( row * width + col )*3;
					int bottom_offset = ( ( height - 1 - row ) * width + col )*3;
					// Swap pixels between the top and bottom rows
					byte  temp = image->data[top_offset];
					image->data[top_offset] = image->data[bottom_offset];
					image->data[bottom_offset] = temp;
					temp = image->data[top_offset+1];
					image->data[top_offset+1] = image->data[bottom_offset+1];
					image->data[bottom_offset+1] = temp;
					temp = image->data[top_offset+2];
					image->data[top_offset+2] = image->data[bottom_offset+2];
					image->data[bottom_offset+2] = temp;
				}
			}
			break;
		default:
