...
static bool
isFaces( ZFILE* zf, strbyte* name, strbyte* fname, strbyte* lname, unsigned int* wp, unsigned int* hp, unsigned int* dp )
{
	strbyte          buf[BUFSIZ];
	...
	w= h= d= 0;
	fname[0]= lname[0]= '\0';
	while ( zgets( ( strbyte* )buf, BUFSIZ - 1, zf ) ) {
		if ( ! strcmp( buf, "\n" ) ) {
			break;
		}
...
Image* facesLoad( strbyte* fullname, ImageOptions* image_ops, bool verbose )
{
	...
	Image*        image;
	strbyte          fname[BUFSIZ];
	strbyte          lname[BUFSIZ];
	unsigned int  w, h, d;
	unsigned int  x, y;
	int           value;
