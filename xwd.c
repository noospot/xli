...
static int isXWD( strbyte* name, ZFILE* zf, XWDHeader* header, int verbose )
{
	GenericXWDHeader gh;
	int a;
	int type = NOT_XWD;
...
static Image* loadXYBitmap( strbyte* name, ZFILE* zf, XWDHeader header, int type )
{
	Image* image;
...
static Image* loadXYPixmap( strbyte* name, ZFILE* zf, XWDHeader header, int type )
{
	Image* image;
	int plane;
	int dlinelen;		/* length of scan line in data file */
...
/* loads a ZPixmap format image.  note that this only supports depths
 * of 4, 8, 16, 24, or 32 bits as does Xlib.  
 * got a 6-bit image, got a problem.  
 * 1-bit images are handled by XYBitmap.
 */

static Image* loadZPixmap( strbyte* name, ZFILE* zf, XWDHeader header, int type )
{
	Image* image;
	int dlinelen;		/* length of scan line in data file */
