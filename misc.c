...
static strbyte* signalName( int sig )
{
	static strbyte buf[32];
...
information about the error is available, sorry.\n" );
	...
	printf( "If running xli again with the -dumpcore flag, then it will core dump\n" );
	printf( "so the problem can be investigated.\n" );
	exit( 1 );
}
...
Image* processImage( DisplayInfo* dinfo, Image* iimage, ImageOptions* options )
{
	...
	if ( options->xzoom || options->yzoom ) {
		...
		if ( !options->colors && RGBP( image ) &&
			...
			compact_cmap( image, globals.verbose );
		}
	...
	if ( RGBP( image ) && !image->rgb.compacted_cmap ) {
...
static bool BEHexTableInitialized = FALSE;
short LEHexTable[256];		/* Little endian conversion value */
static bool LEHexTableInitialized = FALSE;

#define b0000 0			/* things make more sense if seeing them by bit */
#define b0001 1                 // not required with gcc version > FIXME
#define b0010 2
#define b0011 3
#define b0100 4
#define b0101 5
