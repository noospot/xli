...
static int check_png( strbyte* file_name )
{
	ZFILE* zfp;
	byte buf[8];
	int ret;
...
static void describe_png( strbyte* name, png_struct* png, png_info* info )
{
	int bd, ct;
	...
	printf( "%s is a %ux%u PNG image ",
	        name,
	        png_get_image_width( png, info ),
	        png_get_image_height( png, info ) );
	printf( "%s, ",	( ct & PNG_COLOR_MASK_PALETTE ) ? "paletted" :
	        ( ct & PNG_COLOR_MASK_COLOR ) ? "RGB" : "grayscale" );
	printf( "%s", png_get_interlace_type( png, info ) ? "interlaced, " : "" );
	printf( "%d bits deep, ", bd );
...
static void xli_png_read_data( png_struct* png, png_byte* data, png_size_t length )
{
	ZFILE* zfp;

	zfp = ( ZFILE* ) png_get_io_ptr( png );
	if ( zread( zfp, ( byte* ) data, ( int ) length ) != ( long int )length ) {
