...
static int xli_jpg_fill_buf( j_decompress_ptr cinfo )
{
...
static void describe_jpeg( j_decompress_ptr cinfo, strbyte* filename )
{
...
static int xli_jpg_com( j_decompress_ptr cinfo )
{
	long length;
	unsigned int ch;
	...
	while ( --length >= 0 ) {
		...
		/* Emit the byte in a readable form.
...
Image* jpegLoad( strbyte* fullname, ImageOptions* image_ops, bool verbose )
{
	...
	//image->gamma = DEFAULT_DISPLAY_GAMMA;
