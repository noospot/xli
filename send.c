...
static unsigned int bitsPerPixelAtDepth( Display* disp, int scrn, unsigned int depth )
{
	...
	if( !xf || nxf<=0 ) {
		fprintf( stderr, "bitsPerPixelAtDepth: XListPixmapFormats() return null\n" );
		exit( 1 );
	}
	for ( a = 0; a < nxf; a++ )
		if ( xf[a].depth == ( long int )depth ) {
			int bpp;
	...
	for ( a= 0; a < disp->nformats; a++ )
		if ( disp->pixmap_format[a].depth == ( long int )depth ) {
                         ...
                         int depth, int format )
{
	...
	xii->ximage->data = ( strbyte* )lmalloc( image->height * xii->ximage->bytes_per_line );
}
                           ...
                           unsigned int fit, ImageOptions* options )
{
	...
	if( GAMMA_NOT_EQUAL( display_gamma,image->gamma ) && !( BITMAPP( image ) ) ) {
		//fprintf(stderr, "GAMMA_NOT_EQUAL: display:% 2.2f image:% 2.2f\n",display_gamma,image->gamma);
...
retry_direct: /* tag we hit if a DirectColor allocation fails on
     * default colormap */
			...
			if ( ( ( long int )redcolors > visual->map_entries ) ||
			      ( ( long int )greencolors > visual->map_entries ) ||
			      ( ( long int )bluecolors > visual->map_entries ) ) {
			...
			for ( a = 0; ( long int )a < visual->map_entries; a++ ) {
				if ( redbottom < 256 ) {
                 ...
                 unsigned w, unsigned h )
{
	...
	if ( src_x < 0 || src_y < 0 ||
	      src_x + ( long int )w > xii->ximage->width || src_y + ( long int )h > xii->ximage->height ) {
		return;
	}
