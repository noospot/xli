...
#define INIT_IMAGE_OPTS(istr){	\
    ...
    istr.border = (strbyte *) 0;	\
    istr.colordither = FALSE;	\
    istr.colors = 0;		\
    istr.delay = -1;		\
    istr.dither = 0;		\
    istr.expand = 0;		\
    istr.gamma = DEFAULT_DISPLAY_GAMMA;	\
    istr.gray = FALSE;	        \
    istr.merge = FALSE;		\
...
int main( int argc, strbyte* argv[] )
{
	Image* idisp;
	ImageOptions* images;
	...
	/*
	 * the RS/6000 (AIX 3.1) has a new signal, SIGDANGER, which the program get
	...
	for ( i = 0; i < nimages; i += dir ) {
		...
		switch ( switchval ) {
			...
			case '#':
				...
				globals.fullscreen=1-globals.fullscreen;

				winwidth=inew->width;
				winheight=inew->height;
				i--;
				break;
			/* next image */
			case ' ':
			...
			case 'b':
			case 'p':
				/* go load earlier image */
				dir = -1;
				break;

			case '.':
				/* go re-load current image */
				i--;
				break;
			...
			case 'r':
				...
				if ( globals.forall && ( ( globals.onroot && globals.fullscreen ) || globals.fillscreen ) && !io->center ) {
					double wr, hr,f;
