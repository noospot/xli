 ...
 * "format 1" XPixmap file with up to a memory limited number of strbytes
...
/* color table */
static void freeCtable( strbyte** ctable, int ncolors )
{
	int i;
 ...
 */
static strbyte* nword( strbyte* s, strbyte* b )
{
	while ( *s != '\0' && ( *s == ' ' || *s == '\t' ) ) {
		s++;
	}
...
/* number of colors */
/* XPM format type */
static bool isXpixmap( ZFILE* zf, unsigned int* w, unsigned int* h, unsigned int* cpp, unsigned int* ncolors, int* format, strbyte** imagetitle )
{
	strbyte buf[BUFSIZ];
	strbyte what[BUFSIZ];
	strbyte* p;
	...
	*format = *w = *h = *ncolors = 0;
	for ( ; maxlines-- > 0; ) {
		if ( !zgets( ( strbyte* ) buf, BUFSIZ - 1, zf ) ) {
			if ( *imagetitle ) {
...
Image* xpixmapLoad( strbyte* fullname, ImageOptions* image_ops, bool verbose )
{
	...
	unsigned int cpp;	/* strbytes per pixel */
	unsigned int ncolors;	/* number of colors */
	unsigned int depth;	/* depth of image */
	int format;		/* XPM format type */
	strbyte** ctable = NULL;	/* temp color table */
	...
	for ( donecmap = 0; !donecmap; ) {	/* until we have read a colormap */
		...
		znocache( zf );
		ctable = ( strbyte** ) lmalloc( sizeof( strbyte* ) * ncolors );
		memset( ctable, 0, sizeof( strbyte* ) * ncolors );
		...
		for ( a = 0; a < ncolors; a++ ) {
			...
			*( ctable + a ) = ( strbyte* ) lmalloc( sizeof( strbyte* ) * ncolors );
			...
			/* Read cpp strbyteacters in as the color symbol */
			for ( b = 0; b < cpp; b++ ) {
				if ( ( c = zgetc( zf ) ) == '\\' ) {
					c = zgetc( zf );
				}
	...
	cmin = 1 << ( sizeof( int ) * 8 - 2 );	/* Hmm. should use INT_MAX */
	cmax = 0;
	for ( a = 0; a < ncolors; a++ ) {
		int val;
