...
static byte** scan;	/* buffer for input data */
...
Image* rleLoad( strbyte* fullname, ImageOptions* image_ops, bool verbose )
{
	...
	if ( ptype != SC_M && ptype != BW_NM ) {
		scan = ( byte** ) lmalloc( sv_globals.sv_ncolors *
		                                    sizeof( byte* ) );
		for ( i = 0; i < sv_globals.sv_ncolors; i++ ) {
			scan[i] = ( byte* ) lmalloc( x_len );
		}
	}
