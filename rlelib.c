 ...
 *
 * This software may not be included in a program or other software product
 * without supplying the source, or without informing the end-user that the
 * source is available for no extra charge.
 *
 * If modifying this software, a notice should be included, giving the
...
struct sv_globals sv_globals = {
	...
	NULL,			/* output file */
	{ 7 },			/* RGB channels only */
sv_private: {               /* initialise union */
		.get = {                /* Initialize the 'get' member */
			scan_y: 0,
			vert_skip: 0,
			is_eof: 0,
			is_seek: 0,
 ...
 *             ysize;
 *     byte    rgb[3];			* Background color
 *     byte    map;			* flag for map presence
 * }
 *
 ...
 */
static strbyte*
match( register strbyte* n, register strbyte* v )
{
	for ( ; *n != '\0' && *n != '=' && *n == *v; n++, v++ )
		;
	if ( *n == '\0' || *n == '=' ) {
 ...
 */
strbyte*
rle_getcom( strbyte* name, struct sv_globals* globals )
{
	strbyte** cp;
	strbyte* v;
...
int rle_get_setup( struct sv_globals* globals )
{
	...
	if ( setup.h_flags & H_COMMENT ) {
		...
		comment_buf = ( strbyte* )lmalloc( ( unsigned ) evenlen );
		if ( comment_buf == NULL ) {
			fprintf( stderr,
		...
		globals->sv_comments =
		      ( strbyte** )lmalloc( ( unsigned )( i * sizeof( strbyte* ) ) );
		if ( globals->sv_comments == NULL ) {
			fprintf( stderr,
...
void rle_free_setup( struct sv_globals* globals )
{
	if( globals->sv_bg_color != NULL ) {
		lfree( ( byte* )globals->sv_bg_color );
	}
	if( globals->sv_cmap != NULL ) {
		lfree( ( byte* )globals->sv_cmap );
	}
	if( globals->sv_comments != NULL ) {
		if( *globals->sv_comments != NULL ) {
			lfree( ( byte* )*globals->sv_comments );
		}
		lfree( ( byte* )globals->sv_comments );
	}
...
int rle_getrow( struct sv_globals* globals, rle_pixel** scanline )
{
	...
	if ( globals->sv_background == 2 ) {
		if ( globals->sv_alpha && SV_BIT( *globals, -1 ) ) {
			memset( (byte* )scanline[-1], 0, globals->sv_xmax + 1 );
		}
		for ( nc = 0; nc < globals->sv_ncolors; nc++ )
			if ( SV_BIT( *globals, nc ) )
				memset( ( byte* )scanline[nc],
				        globals->sv_bg_color[nc], globals->sv_xmax+1 ); // FIXME globals->sv_bg_color[nc] is an int
	}
