...
// avoid overflowing  multiplications
static unsigned int ovmul( unsigned int a, unsigned int b )
{
	unsigned int r;
	// if with gcc or clang:   bool overflow = __builtin_mul_overflow(a, b, &result);
	r = a * b;
	if ( a != 0 && ( r / a != b ) ) {
		memoryExhausted();
	}
...
Image* newBitImage( unsigned width, unsigned height )
{
	...
	*( image->rgb.red + 1 ) = *( image->rgb.green + 1 ) = *( image->rgb.blue + 1 ) = 0;
	...
	image->depth = 1;
	linelen = ( ( width + 7 ) / 8 );
	image->data = ( byte* ) lcalloc( ovmul( linelen, height ) );
...
Image* newRGBImage( unsigned int width, unsigned int height, unsigned int depth )
{
	...
	image->pixlen = pixlen;
	image->data =
	      ( byte* ) lmalloc( ovmul( ovmul( width, height ), pixlen ) );
...
Image* newTrueImage( unsigned int width, unsigned int height )
{
	...
	image->pixlen = 3;
	image->data =
	      ( byte* ) lmalloc( ovmul( ovmul( width, height ), 3 ) );
