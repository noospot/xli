...
static void setViewportColormap( Display* disp, int scrn, Visual* visual )
{
	 ...
	 */
		...
		XChangeProperty( disp, ViewportWin, cmap_atom, XA_WINDOW, 32,
		                 PropModePrepend, ( byte* ) cmap_windows, 2 );
	}
}

/* attempts to convert an image title into a reasonable icon name
 */
static strbyte* iconName( strbyte* s )
{
	static strbyte buf[BUFSIZ];
	strbyte* t;
...
static Visual* bestVisualOfClassAndDepth( Display* disp, int scrn, int class, unsigned int depth )
{
	...
	/* not sure what to do if info has more than one visual of a particular
...
/* tries to determine the best available visual to use for a particular
 * image
 */
static void bestVisual( Display* disp, int scrn, Image* image, Visual** rvisual, unsigned int* rdepth )
{
	...
	for ( a = 0; a < screen->ndepths; a++ ) {
		if ( screen->depths[a].nvisuals &&
		      ( ( !depth
		          || ( ( depth < ( long int )image->depth ) && ( screen->depths[a].depth > depth ) )
		          || ( ( screen->depths[a].depth >= ( long int )image->depth )
...
strbyte imageInWindow( DisplayInfo* dinfo, Image* image, ImageOptions* options, int argc, strbyte** argv )
{
	Display* disp = dinfo->disp;
	int scrn = dinfo->scrn;
	...
	swa_view.background_pixel = BlackPixel( disp, scrn );
	...
	if ( pixmap == None ) {
		...
		swa_img.background_pixel = BlackPixel( disp, scrn );
	...
	// window title
	{
		const Atom NET_WM_NAME = XInternAtom( disp, "_NET_WM_NAME", 0 );
		const Atom UTF8_STRING = XInternAtom( disp, "UTF8_STRING", 0 );
		const Atom NET_WM_ICON_NAME = XInternAtom( disp, "_NET_WM_ICON_NAME", 0 );
		if ( options->title ) {	/* Option overides title */
			...
			XChangeProperty( disp, ViewportWin,
			                 NET_WM_NAME, UTF8_STRING, 8, PropModeReplace,
			                 ( byte* ) options->title, strlen( options->title ) );

			XChangeProperty( disp, ViewportWin,
			                 NET_WM_ICON_NAME, UTF8_STRING, 8, PropModeReplace,
			                 ( byte* ) options->title, strlen( options->title ) );
			...
			XChangeProperty( disp, ViewportWin,
			                 NET_WM_NAME, UTF8_STRING, 8, PropModeReplace,
			                 ( byte* ) image->title, strlen( image->title ) );

			XChangeProperty( disp, ViewportWin,
			                 NET_WM_ICON_NAME, UTF8_STRING, 8, PropModeReplace,
			                 ( byte* ) image->title, strlen( image->title ) );
		} else {
			XStoreName( disp, ViewportWin, "" );
			XSetIconName( disp, ViewportWin, iconName( ( strbyte* ) 0 ) );
	...
	//
	Atom atom_wm_state = XInternAtom( disp, "_NET_WM_STATE", False );
	Atom atom_fullscreen = XInternAtom( disp, "_NET_WM_STATE_FULLSCREEN", False );

	// fullscreen
	if ( globals.fullscreen || globals.fillscreen ) {

		/* a ClientMessage event to go fullscreen */
		XEvent xev;
		memset( &xev, 0, sizeof( xev ) );
		xev.xclient.type = ClientMessage;
		xev.xclient.serial = 0;
		xev.xclient.send_event = True;
		xev.xclient.window = ViewportWin;
		...
		XSendEvent( disp, RootWindow( disp, scrn ), False,
	...
	for ( ;; ) {
		...
		switch ( event.any.type ) {
			...
			case KeyPress: {
				...
				if( ( ( ( long int )image->width > winwidth ) || ( ( long int )image->height > winheight ) ) ) {
					switch ( ks ) {
						case XK_Left:
						case XK_KP_Left:
							pixx += winwidth/10+1;
			...
			case ClientMessage:
				...
				if ( ( event.message.window == ViewportWin ) &&
				      ( event.message.data.l[0] == ( long int )delete_atom ) ) {
					cleanUpImage( disp, scrn, swa_view.cursor, pixmap, image, xii );
					return ( 'q' );
				}
				break;
